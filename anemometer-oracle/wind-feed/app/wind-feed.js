...
.then(data => {
	...
	let { time, summary, windSpeed } = results.currently;
	windSpeed = windSpeed * 1000;

	let location = `${latitude},${longitude}`;

	if (isNaN(time) || isNaN(windSpeed) || summary == undefined)
	{
		throw new Error("invalid results");
	}

	let iexeccallback = ethers.utils.defaultAbiCoder.encode(['uint256', 'string', 'string', 'uint256'], [time, location, summary, windSpeed]);
	...
	console.log(`- Success: ${time} ${summary} ${windSpeed}`);
})
.catch(error => {
	fs.writeFile(
		errorFilePath,
